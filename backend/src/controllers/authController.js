...
const cambiarPassword = async (req, res) => {
  try {
    ...
    const u = await query("SELECT password_hash FROM usuarios WHERE id=$1", [
      req.user.id,
    ]);
...
const inscripcionPublica = async (req, res) => {
  ...
  try {
    ...
    const alumnoRes = await client.query(
      `INSERT INTO alumnos (usuario_id, nivel_id, tipo_clase)
       VALUES ($1, $2, $3)
       RETURNING id`,
    ...
    await client.query("INSERT INTO fichas_tecnicas (alumno_id) VALUES ($1)", [
      alumnoId,
    ]);

    if (grupo_id || paquete_id) {
      await client.query(
        `INSERT INTO inscripciones (alumno_id, grupo_id, paquete_id, dia_pago)
         VALUES ($1, $2, $3, $4)`,
    ...
    const token = jwt.sign(
      { id: usuarioId, rol_id: 3 },
      process.env.JWT_SECRET,
      { expiresIn: "7d" },
    );

    res.status(201).json({
      mensaje: "¡Bienvenido a Cancha 10!",
      token,
      usuario: {
        id: usuarioId,
        nombre,
