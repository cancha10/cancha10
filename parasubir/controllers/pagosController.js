...
const registrar = async (req, res) => {
  try {
    const { alumno_id, inscripcion_id, sesion_id, tipo, monto, periodo_inicio, periodo_fin, metodo_pago, notas } = req.body;
    if (!alumno_id || !monto) return res.status(400).json({ error: 'alumno_id y monto requeridos' });
    const result = await query(
      ...
      [alumno_id, inscripcion_id||null, sesion_id||null, tipo||'mensual', monto, periodo_inicio||null, periodo_fin||null, metodo_pago||null, notas||null]
    );
...
const subirComprobante = async (req, res) => {
  try {
    ...
    const resultado = await subirArchivo(req.file.buffer, `pago-${id}-${Date.now()}`);
    const update = await query(`UPDATE pagos SET comprobante_url=$1, comprobante_nombre=$2, comprobante_fecha=NOW() WHERE id=$3 RETURNING *`, [resultado.secure_url, req.file.originalname, id]);
    if (!update.rows.length) return res.status(404).json({ error: 'Pago no encontrado' });
...
module.exports = { listar, registrar, pendientes, misPagos, actualizarEstado, subirComprobante };
