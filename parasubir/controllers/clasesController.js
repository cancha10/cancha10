...
const sesionesDelDia = async (req, res) => {
  try {
    ...
    for (const c of clases.rows) {
      let sesion = await query('SELECT id FROM sesiones WHERE clase_id=$1 AND fecha=$2', [c.clase_id, fecha]);
      if (!sesion.rows.length) sesion = await query('INSERT INTO sesiones (clase_id, fecha) VALUES ($1,$2) RETURNING id', [c.clase_id, fecha]);
      const sesionId = sesion.rows[0].id;
      const inscritos = await query(`SELECT COUNT(*) AS total FROM inscripciones WHERE grupo_id=$1 AND estado='activa'`, [c.grupo_id]);
      sesiones.push({
        id: sesionId, clase_id: c.clase_id, grupo_id: c.grupo_id, grupo: c.grupo, tipo: c.tipo,
        nivel: c.nivel, capacidad: c.capacidad, hora_inicio: c.hora_inicio, hora_fin: c.hora_fin,
...
const alumnosDeSesion = async (req, res) => {
  try {
    ...
    if (!sesionInfo.rows.length) return res.status(404).json({ error: 'Sesión no encontrada' });
...
const registrarAsistencia = async (req, res) => {
  try {
    ...
    if (!Array.isArray(asistencias)) return res.status(400).json({ error: 'asistencias debe ser array' });
    for (const a of asistencias) {
      await query(
        `INSERT INTO asistencia (sesion_id, alumno_id, estado) VALUES ($1,$2,$3)
         ON CONFLICT (sesion_id, alumno_id) DO UPDATE SET estado=$3`,
...
const crearClase = async (req, res) => {
  try {
    ...
    if (!grupo || !dias?.length || !hi || !hf) return res.status(400).json({ error: 'grupo, dias, hi y hf requeridos' });
    const grupoRes = await query('INSERT INTO grupos (nombre, nivel_id, tipo, capacidad) VALUES ($1,$2,$3,$4) RETURNING id', [grupo, nivel_id||null, tipo||'grupal', capacidad||null]);
    const grupoId = grupoRes.rows[0].id;
    for (const dia of dias) await query('INSERT INTO clases (grupo_id, dia_semana, hora_inicio, hora_fin) VALUES ($1,$2,$3,$4)', [grupoId, dia, hi, hf]);
...
const editarClase = async (req, res) => {
  try {
    ...
    const { grupo, dias, hi, hf, tipo, nivel_id, capacidad } = req.body;
    await query('UPDATE grupos SET nombre=$1, nivel_id=$2, tipo=$3, capacidad=$4 WHERE id=$5', [grupo, nivel_id||null, tipo||'grupal', capacidad||null, grupoId]);
...
module.exports = { horario, sesionesDelDia, alumnosDeSesion, registrarAsistencia, crearClase, editarClase, eliminarClase };
