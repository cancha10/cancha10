...
const crear = async (req, res) => {
  try {
    const { alumno_id, grupo_id, paquete_id, fecha_inicio, dia_pago } = req.body;
    ...
    const result = await query(
      ...
      [alumno_id, grupo_id||null, paquete_id||null, fecha_inicio||new Date().toISOString().split('T')[0], dia_pago||null]
    );
    res.status(201).json({ mensaje: 'Inscripción creada', inscripcion: result.rows[0] });
  } catch (err) {
...
const renovar = async (req, res) => {
  try {
    ...
    const result = await query(`UPDATE inscripciones SET fecha_inicio=$1, estado='activa' WHERE id=$2 RETURNING *`, [fecha_inicio||new Date().toISOString().split('T')[0], id]);
    if (!result.rows.length) return res.status(404).json({ error: 'Inscripción no encontrada' });
...
const actualizarDiaPago = async (req, res) => {
  try {
    ...
    if (!dia_pago || dia_pago < 1 || dia_pago > 31) return res.status(400).json({ error: 'Día de pago debe ser entre 1 y 31' });
    const result = await query('UPDATE inscripciones SET dia_pago=$1 WHERE id=$2 RETURNING *', [dia_pago, id]);
    if (!result.rows.length) return res.status(404).json({ error: 'Inscripción no encontrada' });
    res.json({ mensaje: 'Día de pago actualizado', inscripcion: result.rows[0] });
  } catch (err) {
...
const listarPaquetes = async (req, res) => {
  try {
    const result = await query('SELECT * FROM paquetes WHERE activo = TRUE ORDER BY tipo, clases_semana');
    res.json(result.rows);
  } catch (err) {
...
const crearPaquete = async (req, res) => {
  try {
    const { nombre, tipo, clases_semana, precio_mensual, precio_clase, precio_cancha, descripcion } = req.body;
    if (!nombre || !tipo) return res.status(400).json({ error: 'Nombre y tipo requeridos' });
    if (tipo === 'grupal' && !precio_mensual) return res.status(400).json({ error: 'precio_mensual requerido para paquetes grupales' });
    if (tipo === 'particular' && !precio_clase) return res.status(400).json({ error: 'precio_clase requerido para paquetes particulares' });
    const result = await query(
      ...
      [nombre, tipo, clases_semana||null, precio_mensual||null, precio_clase||null, precio_cancha||0, descripcion||null]
    );
    res.status(201).json({ mensaje: 'Paquete creado', paquete: result.rows[0] });
  } catch (err) {
...
const editarPaquete = async (req, res) => {
  try {
    ...
    const result = await query(
      ...
      [nombre, tipo, clases_semana||null, precio_mensual||null, precio_clase||null, precio_cancha||0, descripcion||null, id]
    );
...
const eliminarPaquete = async (req, res) => {
  try {
    await query('UPDATE paquetes SET activo=FALSE WHERE id=$1', [req.params.id]);
...
module.exports = { listar, crear, renovar, actualizarDiaPago, listarPaquetes, crearPaquete, editarPaquete, eliminarPaquete };
